  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <label class="el-form-item-label">HiveSchema名称</label>
        <el-input v-model="query.dwSchemaName" clearable placeholder="HiveSchema名称" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        <label class="el-form-item-label">HiveSchema编码</label>
        <el-input v-model="query.dwSchemaCode" clearable placeholder="HiveSchema编码" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        <label class="el-form-item-label">数据库类型</label>
        <el-input v-model="query.dbType" clearable placeholder="数据库类型" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        ...
        <el-input v-model="query.dwServerIp" clearable placeholder="服务地址" style="width: 185px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
        <el-date-picker
        ...
        <el-date-picker
          ...
          class="date-item"
          value-format="yyyy-MM-dd HH:mm:ss"
          start-placeholder="submitTimeStart"
          end-placeholder="submitTimeEnd"
        />
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          <el-form-item label="HiveSchema名称" prop="dwSchemaName">
            <el-input v-model="form.dwSchemaName" style="width: 370px;" />
          </el-form-item>
          <el-form-item label="HiveSchema编码">
            <el-input v-model="form.dwSchemaCode" style="width: 370px;" />
          </el-form-item>
          <el-form-item label="数据库类型" prop="dbType">
            <el-radio v-model="form.dbType" v-for="item in dict.db_type" :key="item.id" :label="item.value">{{ item.label }}</el-radio>
          </el-form-item>
          <el-form-item label="所属业务线" prop="businessLine">
            <el-radio v-model="form.businessLine" v-for="item in dict.business_line" :key="item.id" :label="item.value">{{ item.label }}</el-radio>
          </el-form-item>
          ...
          <el-form-item label="使用状态">
            <el-radio v-model="form.useFlag" v-for="item in dict.dw_use_status" :key="item.id" :label="item.value">{{ item.label }}</el-radio>
          </el-form-item>
          ...
          <el-form-item label="元数据库类型">
            <el-radio v-model="form.dwMetastoreType" v-for="item in dict.db_type" :key="item.id" :label="item.value">{{ item.label }}</el-radio>
          </el-form-item>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="dwSchemaName" label="HiveSchema名称" />
        <el-table-column prop="dwSchemaCode" label="HiveSchema编码" />
        ...
        <el-table-column prop="dwServerPort" label="服务端口" />
        <el-table-column prop="dwMetastoreType" label="元数据库类型">
          <template slot-scope="scope">
            {{ dict.label.db_type[scope.row.dwMetastoreType] }}
          </template>
        </el-table-column>
        <el-table-column prop="dwMetastoreUser" label="用户名" />
        <el-table-column prop="dwMetastorePass" label="密码" />
...
<script>
  import crudDwSchemaInfo from '@/api/dwSchemaInfo'
  ...
  export default {
    ...
    dicts: ['db_type', 'business_line', 'dw_use_status', 'table_data_status', 'db_type'],
    cruds() {
      return CRUD({ title: 'DataWarehouse', url: 'api/dwSchemaInfo', sort: 'id,desc', crudMethod: { ...crudDwSchemaInfo }})
    },
    data() {
      return {
        permission: {
        ...
        rules: {
          dwSchemaName: [
            { required: true, message: 'HiveSchema名称不能为空', trigger: 'blur' }
          ],
          dbType: [
        ...
        queryTypeOptions: [
          { key: 'dwSchemaName', display_name: 'HiveSchema名称' },
          { key: 'dwSchemaCode', display_name: 'HiveSchema编码' },
          { key: 'dbType', display_name: '数据库类型' },
          { key: 'businessLine', display_name: '所属业务线' },
          { key: 'useFlag', display_name: '使用状态' },
          { key: 'dwServerIp', display_name: '服务地址' }
        ]
      }
    },
    methods: {
